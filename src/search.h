...
namespace LogSearch
{
    ...
    public:
        explicit FileSearchResults(QString fileName, QString filePath,
                                   QObject* parent = nullptr)
            : QObject{parent}, fileName(fileName), filePath(filePath),
              matches(new QList<Match>())
        {
            connect(
                this, &QObject::destroyed, this,
                [](QObject* obj) { qDebug() << "Results::destroyed" << obj; },
                Qt::QueuedConnection);
        };
    ...
    public:
        ...
        {
            setObjectName(QString("Search: %1 :: #files: %2")
                          .arg(params.searchString)
                          .arg(params.files.length()));
        };
    ...
    public:
        using SearchCompleteSignal =
        std::function<void(const Status, const QSharedPointer<Search>)>;
        using SearchProgressSignal =
        std::function<void(int, const QSharedPointer<Search>)>;

        QSharedPointer<Search> search(SearchParameters params);
        QSharedPointer<Search> search(SearchParameters params, QObject* receiver,
    ...
    private slots:
        ...
        void onSearchCanceled(const QSharedPointer<Search> search,
                              SearchCompleteSignal onCompleteCallback);
        void onSearchFinished(const QSharedPointer<Search> search);
        void onSearchFinished(const QSharedPointer<Search> search,
                              SearchCompleteSignal onCompleteCallback);
        void onSearchProgress(const int progress,
                              const QSharedPointer<Search> search);
        void onSearchProgress(const int progress, const QSharedPointer<Search> search,
                              SearchProgressSignal onProgressCallback);

    private:
        SearchManager(QObject* parent = nullptr) : QObject{parent}
        {
